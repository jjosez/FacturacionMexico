...
namespace FacturaScripts\Plugins\FacturacionMexico\Lib\Infrastructure\StampProviders;
...
class FinkokStampProvider implements StampProviderInterface
{
    ...
    public function cancel(string $uuid, Credential $credential): StampResult
    {
        ...
        $result = $this->quickFinkok->cancel($credential, $cancelDocument);
        $responseDocument = $result->documents()->first();
        ...
        return new StampResult(true, '', '', 'Error al cancelar el cfdi.' . print_r($result->statusCode(), true));
    }
